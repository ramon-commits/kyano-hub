...
export default function VerjaardagenView({ onOpenContact, onSchedule }) {
  ...
  const label = (d) => {
    if (d === 0) return { text: '🎂 VANDAAG!', color: '#dc2626', bg: '#fef2f2' };
    if (d === 1) return { text: 'Morgen', color: '#ea580c', bg: '#fff7ed' };
    if (d <= 7) return { text: `Over ${d}d`, color: '#ec4899', bg: '#fdf2f8' };
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-gray-200 bg-white px-8 py-5">
        <h1 className="text-xl font-semibold text-gray-900">🎂 Verjaardagen</h1>
        <p className="mt-0.5 text-sm text-gray-500">Komende 30 dagen — {birthdays.length} contacten</p>
      </div>
                ...
                return (
                  ...
                  >
                    ...
                    <button onClick={() => onOpenContact?.(b)} className="min-w-0 flex-1 text-left">
                      ...
                      {b.company ? <div className="truncate text-sm text-gray-500">{b.company}</div> : null}
                      <div className="mt-1 text-xs text-gray-500">
                        🎂 {formatDateShort(`${b.birthday}T00:00:00`)} · {b.next_birthday}
                      </div>
                    </button>
                    <Badge color={lab.color} bg={lab.bg} size="lg">{lab.text}</Badge>
